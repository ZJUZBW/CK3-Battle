...
- 兵士军团的**规模上限**：兵士军团可以增加规模，增加规模的费用与招募他们的费用相同。规模上限决定每个兵士军团可以维持的最大规模。
  - 基础规模上限为3，部落时代上限为5（校阅场+2），中世纪早期上限为8（家兵+3），中世纪盛世上限为12（兵士+4），中世纪晚期上限为17（常备军+5）。
  - **建筑加成**：军校+1/2/3，从中世纪早期研究乡堡革新以后才能建造。
  - **不同类型兵士的规模上限加成**
    - 散兵：【散兵】+4/6/8，【人多力量大】：+2
    - 弓箭手：【弓箭手】+4/6/8，【弩兵队长】+4/6/8，【长弓竞赛】+2，【人多力量大】+2
    - 长枪兵：【枪兵队长】+2/4/6
    - 轻骑兵：【掠袭骑兵】+2/4/6，【人多力量大】+2，【亚历山大之铠】：+1
    - 骆驼骑兵：【骆驼骑兵】+2/4/6
    - 重步兵：【先锋】+2/4/6，【东罗马遗产】+2，【2级劫掠传承】+2
    - 重骑兵：【枪骑兵】+2/4/6，【东罗马遗产】+2
    - 弓骑兵：【弓骑兵】+2/4/6
    - 象骑兵：【象骑兵】+1/2/3
...
**最终伤害** = （基础伤害+地形外的固定数值加成）×（1+百分比加成）×（1+总督效率）×（1+勋号骑士军队修正）+ 地形加成

**最终坚韧** = （基础坚韧+地形外的固定数值加成）×（1+百分比加成）×（1+总督效率）×（1+勋号骑士军队修正）+ 地形加成

**最终追击** = （基础追击+地形外的固定数值加成）×（1+百分比加成）×（1+总督效率）×（1+勋号骑士军队修正）+ 地形加成

**最终掩护** = （基础掩护+地形外的固定数值加成）×（1+百分比加成）×（1+总督效率）×（1+勋号骑士军队修正）+ 地形加成
...
- **添油战术**
  - 不像EU4里有硬性战场宽度限制和侧翼范围限制+后备人员的士气每天下降的设定。CK3的交战双方**总是完全承受所有敌人部队造成的伤害**，**所以CK3里的添油战术无论在什么情况下都是坏的选择**。4万征召兵平均分成4分，分别作战以后的总和战斗力变成原来的1/4。两万征召兵可以连续歼灭3队1万人的征召兵，然后跟最后一队1万人的征召兵同归于尽。
- **分兵策略**
  - 由于战场宽度由战斗的最开始决定，如果其他几个部队晚到了，由于战场宽度的限制，也只能在后排吃灰，排队上场。比如说红方4万人跟蓝方2万人在平原交战，红方分为两队（其中一队晚到4天，也就是调动阶段结束，战场宽度已经确定的时候），最初是红方2万人对抗蓝方两万人，战场宽度是2万。这个时候红方第二只部队上场，由于战场宽度的限制，此时也不能造成任何额外伤害。
  - 所以分兵作战除了能减少补给压力以外，没有任何好处。而且会导致发挥不了数量优势。多用后勤专家之类的将领来增加军队的补给时间吧。
...
- 不考虑卡双建筑BUG的话，保守估计可以当成平均100勇武，+900%骑士战斗力，骑士数量为50。
- 此时单个骑士标准战斗力为31622，等于3162个征召兵。50个骑士的部队等于15.81万征召兵的部队（没考虑战场宽度和部队混合）。
  - 像那些20个骑士就爆杀十多万蒙古大军的（添油战术这种另算了），原版是很难很难做到的。

**前期的骑士有多少战斗力**？

- 前期的话可以当成平均20勇武，骑士战斗力+100%，骑士数量10。
- $e=20\times(1+1)\times\sqrt{100\times10}=1265$
- 单个骑士的精锐度为1265，相当于126.5个征召兵。单独成军的10个骑士相当于1265个征召兵。搭配1265个征召兵的话，相当于（1265+1265）×1.2 =3036个征召兵。
...
在之前的战斗力介绍里，我们可以知道，部队真正的战斗力是标准战斗力的平方倍。也就是说部队数量变成2倍，战斗力就会变成原来的4倍；而伤害变成原先的2倍，战斗力只会变成原来的2倍。只有兵种的伤害×坚韧变成原来的4倍，才能抵消2倍的人数优势。
...
此时1个弓骑兵在战斗阶段等于19.7个征召兵，从部落时代玩到中世纪盛世已经可以有一万人弓骑兵了，可以吊打AI的十几万大军。
...
**战场宽度的定义**：在战斗的开始（调动阶段结束）决定整场战斗的战场宽度，战场宽度等于双方军力总和除以 2，然后乘战场宽度系数（与地形有关）。数量超过战场宽度的士兵将处于待命阶段，无法造成伤害。**骑士不占据战场宽度，也不会被战场宽度限制而减少伤害（待测试）**。
...
- 由于战场宽度对于伤害的限制，1个100伤害的士兵会比10个造成10伤害的士兵更优。这也是战象、重骑兵这些兵士的优势之一。

### 战场宽度对战斗力的影响

#### 单一部队

**部队的战斗力等于紫色线条跟x和y轴围成的图形面积（即1/2*标准战斗力的平方）**，由于战场宽度减少的战斗力为阴影区域面积，这个面积占总面积的（1-战场宽度/部队初始军力）<sup>2</sup>。比如战场宽度为初始部队军力的一半，则减少了1/4的战斗力。

![战场宽度对单一部队的影响](images/战场宽度对单一部队的影响.png)

#### 混合部队

**混合系数越高的部队混合构成军队，则军队受到战场宽度的影响越小**。也就是说，战场宽度为初始军力的1/3的时候，真正减少的战斗力会比（1 - 1/3）<sup>2</sup>= 4/9 要少。

- 游戏里战场宽度最多只会限制到初始军力的1/4（即山地、沙漠山地；比如己方10骑士，敌方几万人），最多只能让敌人损失（1 - 1/4）<sup>2</sup>= 49%的战斗力（简单理解，相当于敌人数量变成原来的70%）。或者另一个说法，两个部队的标准战斗力差距30%以上的时候，什么地形都拯救不了你。
- 骑士伤害不受战场宽度限制（未测试），所以骑士流部队基本上不会受到战场宽度限制，带越多兵越强。

![战场宽度对混合部队的影响](images/战场宽度对混合部队的影响.png)
