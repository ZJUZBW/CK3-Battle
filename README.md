...
**标准战斗力如何预测战斗阶段的结果**：标准战斗力大的一方会获得战斗胜利。假设红方A和蓝方B交战，将两方的标准战斗力平方以后相减，再开平方根即可得到剩余的标准战斗力。公式如下：

- $c_{|A-B|}=\sqrt{|c_A^2-c_B^2|}$
- 从这个公式可以看出来，部队实际的战斗力跟
...
**不同兵种的部队合并后，标准战斗力如何叠加**：以兵种A和兵种B的合并为例。

- $c_{A+B}=\sqrt{c_A^2+c_B^2+2(\alpha +1)c_Ac_B}$
  ...
  - 精锐度相同或者攻击系数相同的部队之间的混合系数为0。所以相同部队的标准战斗力叠加公式如下：
    - $c_{A+B}=\sqrt{c_A^2+c_B^2+2c_Ac_B}=\sqrt{(c_A+c_B)^2}=c_A+c_B$
  - 比如，10个10勇武骑士，精锐度为316.2，攻击系数为3.162，标准战斗力为3162，单独作战的时候相当于316.2个征召兵。搭配2000征召兵，精锐度为10，攻击系数为1，标准战斗力为20000。
    - 精锐度比值e<sub>A/B</sub>=31.62，攻击系数比值k<sub>A/B</sub>=3.162，计算出混合系数为2.9。
    - 所以合并以后的标准战斗力为27871，相当于2787.1个征召兵。
