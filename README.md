...
- 假设兵种A的伤害为D<sub>A</sub>，坚韧为T<sub>A</sub>，在交战第t轮时的军力为S<sub>A</sub>（t），初始军力为S<sub>A</sub>（0）。
- 同理，兵种B的伤害为D<sub>B</sub>，坚韧为T<sub>B</sub>，在交战第t轮的军力为S<sub>B</sub>（t），初始军力为S<sub>B</sub>（0）。
...
<img src="http://latex.codecogs.com/gif.latex?\\{\mbox{(5) }}(S_A(0)^2-S_A(t)^2)*E_A=(S_B(0)^2-S_B(t)^2)*E_B">

其实这个方程跟**兰彻斯特作战模型里的平方律**相似，这个方程有简单的结论

- 交战一方的有效战斗力，正比于其军力的平方（S<sup>2</sup>）与每一战斗单位的**装备性能E**（伤害D×坚韧T）的乘积，称其为**平方律**。
...
**军队的有效战斗力还可以有如下定义：即军队被全部歼灭之前能够造成的总伤害**。
