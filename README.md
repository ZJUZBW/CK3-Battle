...
$$伤害=(（基础伤害+文化革新加成）×（1+面板加成）×（1+勋号骑士加成）+地形加成）×（1+优势$$
...
兵士（Men-at-Arm），简称MAA，可以理解为定期操练、装备良好的专业部队。每种类型的兵士都克制至少一种其他类型的兵士，若军团被克制，其伤害下降，伤害同军团**规模**之差成比例。兵士军团每月以其最大规模的10%进行补员。
