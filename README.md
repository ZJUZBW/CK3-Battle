...
**最终伤害** =【（基础伤害+固定数值加成）×（1+百分比加成）×（1+勋号骑士百分比加成）+地形加成】×兵士克制×优势差加成×战场宽度限制
...
- **从上述公式可以看出，真实的部队战斗力实际上与标准战斗力的平方成正比（也可以直接当成标准战斗力的平方，具体数值请看附录）。用标准战斗力的好处是数值小，而且跟部队人数为线性关系，易于理解，而且不影响判断输赢**。
...
- $c_{A+B}=\sqrt{c_A^2+c_B^2+2(\alpha +1)c_Ac_B}=\sqrt{(c_A+c_B)^2+2\alpha c_Ac_B}$
  ...
  - **关于混合系数，有以下结论**：
    - 在兵种混合时，数量少的高精锐度单位（即e更高）应该更多侧重伤害。高攻低血的高质量单位在混合时会让整个军队的有效战斗力提高，低攻高血的高质量单位则相反。
    - 在兵种混合时，数量多的低精锐度单位（即e更低）应该更多侧重坚韧，以便更好得充当肉盾。
  - **举例**，10个10勇武骑士，精锐度为316.2，攻击系数为3.162，标准战斗力为3162，单独作战的时候相当于316.2个征召兵。搭配2000征召兵，精锐度为10，攻击系数为1，标准战斗力为20000。
    - 精锐度比值e<sub>A/B</sub>=31.62，攻击系数比值k<sub>A/B</sub>=3.162，计算出混合系数为2.9。
    - 所以合并以后的标准战斗力为27871，相当于2787.1个征召兵，比2000+316.2要大很多。
    - **当混合部队之间的标准战斗力相同的时候，部队之间混合系数对战斗力的影响最大，想要靠混合部队来增加军队战斗力的话，尽量让部队之间战斗力相近**。
      - 比如10个10勇武骑士，精锐度为316.2，攻击系数为3.162，标准战斗力为3162。跟标准战斗力为3162的征召兵混合。合并以后的标准战斗力为8831，等于原先的1.4倍。
...
![image-20230828175243453](https://github.com/ZJUZBW/CK3-Battle/blob/a6f33ebee6592fc15af8ef20e4f3e04589bb913e/image-20230828175243453.png)
