...
- 单一部队的标准战斗力为c，等于部队的精锐度 e × 部队的军力 S，$c=eS$
- 定义混合部队的标准战斗力，$c=\sum{e_iS_i}$，即有$c_{A+B}=e_AS_A+e_BS_B$，面板战斗力（预估的实际战斗力）为$C=\frac {1} {2}c^2$，在这样折算以后，不会影响战斗力的大小关系。
...
这个式子的最左边为面板战斗力C=1/2×c<sup>2</sup>，

#### 混合系数

设混合系数$\alpha=\frac{T_{B}}{T_{A}+T_{B}}\cdot\frac{e_{A}}{e_{B}}+\frac{T_{A}}{T_{A}+T_{B}}\cdot\frac{e_{B}}{e_{A}}-1$，当α＞0时，保证c不变的情况下，将兵种A部分替换为兵种B，就有C<sub>A+B</sub>＞C。
...
- 在兵种混合时，高精锐度单位（即e更高）应该更多侧重伤害。高攻低血的高质量单位在混合时会让整个军队的实际战斗力提高，低攻高血的高质量单位则相反。
- 在兵种混合时，低精锐度单位（即e更低）应该更多侧重坚韧，以便更好得充当肉盾。

#### 理想情况下的兵种混合

从（18）式中，可以看出，当$\frac{1}{2}c=e_AS_A=e_BS_B$时，即混合兵种双方的标准战斗力相等时，混合系数α对实际战斗力的影响最大。
...
- 极限条件下（即一个兵种精锐度接近无穷大，军力接近无穷小；相当于另一个低精锐度兵种全部死亡之前，这个兵种都能维持100%军力，即造成满额伤害），混合兵种的实际战斗力接近面板战斗力的$\frac{1}{2}+\frac{k_{A/B}}{2}$倍。
- 游戏中当骑士与轻步兵组合时，k<sub>A/B</sub>最大为4，最理想情况下（即混合的骑士标准战斗力和轻步兵标准战斗力相等，且骑士的标准战斗力远大于轻步兵的标准战斗力），实际战斗力会接近预估的实际战斗力的2.5倍。
- 由于骑士的k是最大的，而且精锐度为e也是最大，所以骑士跟所有部队混合都会发挥更强的战斗力。
- 前期配合征召兵的时候，优先选择精锐度e更高的兵士；精锐度e差不多的情况下，优先选择攻击系数k更大的。如果不考虑骑士的话，长枪兵配合征召兵反而会导致实际战斗力不如面板战斗力（但是有骑士作为输出手就不一样了）。
...
以伤害举例，$$伤害=(（基础伤害+文化革新加成）×（1+面板加成）×（1+勋号骑士加成）+地形加成）×（1+优势差×2%）$$
